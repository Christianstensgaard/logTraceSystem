...
public class Package{
  ...
  const int pos_ClientName = 6;
  const int Pos_TaskName = 8;
  const int Pos_Id = 10;
  const int Pos_Payload = 12;
  ...
  public static bool EqualClientName(byte[] t1, byte[] t2){
    int t1Start = GetPosition(pos_ClientName, t1);
    int t2Start = GetPosition(pos_ClientName, t2);

    int t1size = BitConverter.ToInt32(t1, t1Start);
    int t2size = BitConverter.ToInt32(t2, t2Start);
    ...
    {
      if(t1[t1Start+i] != t2[t2Start+i])
        return false;
    }

    return true;
  }
  public static bool EqualTaskName(byte[] t1, byte[] t2){
    int t1Start = GetPosition(Pos_TaskName, t1);
    int t2Start = GetPosition(Pos_TaskName, t2);
  ...
  static int GetPosition(int pos, byte[] buffer){
